...
if ( $exec -eq "exec=true" )
{
  if ( $target -eq "target=rp1" )
  {
    ...
    &"D:\Program Files\qemu\qemu-system" -M raspi2 -kernel output/kernel.img -drive file=SDCard/SDCard.img,if=sd,format=raw -serial stdio
  }
  elseif ( $target -eq "target=rp2" )
  {
    Write-Host "         P I    2 ' S     U A R T 0          "
    Write-Host "= = = = = = = = = = = = = = = = = = = = "
    &"D:\Program Files\qemu\qemu-system" -M raspi2 -kernel output/kernel7.img -drive file=SDCard/SDCard.img,if=sd,format=raw -serial stdio
  }
  elseif ( $target -eq "target=rp3" )
  {
    Write-Host "         P I   3 ' S     U A R T 0          "
    Write-Host "= = = = = = = = = = = = = = = = = = = = "
    & "D:\Program Files\qemu\qemu-system-aarch64" -M raspi3 -kernel output/kernel8.img -drive file=SDCard/SDCard.img,if=sd,format=raw -serial stdio
  }
}
